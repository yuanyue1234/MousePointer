...
- 勾选“自启动并保留后台”后，会写入 HKCU Run 项和启动文件夹快捷方式；若系统允许，会额外创建任务计划程序登录触发项。关闭窗口会保留托盘后台，托盘左键打开窗口，右键菜单提供“打开”和“退出”。
...
- 安装模式支持生成安装版：运行 `鼠标指针配置生成器_安装程序.exe` 会安装到 `%LOCALAPPDATA%\Programs\MouseCursorPointerManager`，并在安装目录放置 `卸载鼠标指针配置管理器.exe`。
...
```powershell
.\.venv\Scripts\python.exe -m PyInstaller --noconsole --windowed --onefile --clean --name 鼠标指针配置生成器_绿色程序 --icon "assets\app.ico" --add-data "assets;assets" --add-data "icon终.png;." --add-data "icon.png;." --add-data "1439978017.svg;." main.py
```
...
dist\鼠标指针配置生成器_安装程序.exe
```

## GitHub Release

推送 `v*` 标签时，GitHub Actions 会把 `release-assets/鼠标指针配置生成器_绿色程序.exe` 和 `release-assets/鼠标指针配置生成器_安装程序.exe` 上传到 Releases。
